# 👨🏻‍💻 Vinícius Souza Silva

**`Estudante de ADS | Futuro em Cibersegurança`**

Me chamo Vinícius, sou estudante de **Análise e Desenvolvimento de Sistemas na Fatec Campinas** e apaixonado por tecnologia. Estou construindo minha base em **programação (C, Python, SQL, HTML, CSS, JavaScript)** e em **infraestrutura de redes e sistemas operacionais**, com objetivo de seguir carreira em **Segurança da Informação**.  

Além da faculdade, concluí os cursos de **Informática Master** (Microlins) e **Analista em Suporte Técnico** (MicroPro), que me proporcionaram uma sólida base teórica em ferramentas corporativas, montagem de computadores, redes e servidores. Esses conhecimentos têm sido fundamentais para compreender a estrutura da tecnologia e suas aplicações. Paralelamente, venho aprofundando meus estudos de forma independente pela internet.

---

### 🛠️ Tecnologias que pratico

<img align="left" alt="C" title="C" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/c/c-original.svg"/>
<img align="left" alt="Python" title="Python" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg"/>
<img align="left" alt="HTML" title="HTML" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg"/>
<img align="left" alt="CSS" title="CSS" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg"/>
<img align="left" alt="JavaScript" title="JavaScript" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg"/>
<img align="left" alt="SQL" title="SQL" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg"/>

<br/>
<br/>

---

### 📂 Projetos e Estudos
- **Em desenvolvimento**
  
---

### 🎯 Objetivo
Construir uma carreira sólida em **Tecnologia da Informação**, e evoluir para a área de **Cibersegurança**.  

---

### 📊 Estatísticas

<p>
  <img 
    align="left" 
    alt="GitHub Stats" 
    height="200" 
    style="padding-right: 10px;" 
    src="https://github-readme-stats.vercel.app/api?username=SEUUSUARIO&show_icons=true&theme=tokyonight&include_all_commits=true&locale=pt-br" 
  />

  <img 
    align="left" 
    alt="Top Langs" 
    height="200" 
    src="https://github-readme-stats.vercel.app/api/top-langs/?username=SEUUSUARIO&theme=tokyonight&layout=compact&custom_title=Tecnologias&langs_count=9" 
  />
</p>
